...
def hill_climbing_with_sideways_move(cube):
    ...
    while iterators != 1000:  # Continue until termination condition is met
        ...
        if best_successor is not None:
            ...
            # Log the swap positions
            print(f"Swapped: {positions[0]} with {positions[1]}")
            ...
            if same_score_successors:
                ...
                # Log the swap that would have occurred for a random successor
                random_positions = np.unravel_index(np.random.randint(0, n**3), (n, n, n)), \
                                   np.unravel_index(np.random.randint(0, n**3), (n, n, n))
                print(f"Swapped: {random_positions[0]} with {random_positions[1]}")
                sideways_moves += 1
            else:
...
# Jalankan algoritma Hill-Climbing with Sideways Move
final_cube = hill_climbing_with_sideways_move(initial_cube)
end_time = time.time()
