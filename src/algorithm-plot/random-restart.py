...
# Fungsi untuk menjalankan algoritma Steepest Ascent Hill-Climbing dengan Random Restart
def random_restart_hill_climbing(cube, max_restarts):
    n = cube.shape[0]
    ...
    all_scores = []  # Menyimpan semua nilai objective function dari setiap restart

    print(f"Initial score: {best_score}")

    # Hill-climbing untuk cube yang diberikan pada awalnya sebelum restart
    while restart_count < max_restarts:
        current_scores = []  # Menyimpan nilai objective function untuk setiap iterasi
        ...
        if restart_count == 0:
            ...
            # Generate random cube untuk setiap restart (kecuali untuk percobaan pertama)
        ...
        while True:  # Hill-climbing sampai tidak ada perbaikan lebih lanjut
            successors = get_successors(current_cube)
            best_neighbor_score = current_score
            best_successor = None

            # Evaluasi semua tetangga dan pilih yang terbaik
            ...
            # Jika tetangga yang lebih baik ditemukan, pindah ke sana
            if best_successor is not None:
                ...
                # Jika tidak ada tetangga yang lebih baik, mencapai local optimum
                print(f"No better neighbor found, local optimum reached with score: {current_score}")
                break

        # Tambahkan hasil skor setiap iterasi dari restart saat ini ke daftar all_scores
        all_scores.append(current_scores)

        # Cek jika solusi saat ini adalah yang terbaik sejauh ini
        if current_score > best_score:
            best_cube = current_cube
            best_score = current_score
            print(f"New best solution found with score: {best_score}")

        # Lanjut ke restart berikutnya
        restart_count += 1

    print(f"\nFinal best score after {max_restarts} restarts: {best_score}")

    # Plot nilai objective function untuk setiap restart
    plt.figure(figsize=(10, 6))
    for i, scores in enumerate(all_scores):
        plt.plot(scores, label=f'Restart {i + 1}')
    plt.xlabel('Iterasi')
    plt.ylabel('Nilai Objective Function')
    plt.title('Nilai Objective Function terhadap Iterasi untuk Setiap Restart')
    plt.legend()
    plt.show()
...
final_cube_random_restart = random_restart_hill_climbing(initial_cube, max_restarts=4)
end_time = time.time()
